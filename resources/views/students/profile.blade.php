...
<div class="flex gap-3  items-stretch">
        ...
        <div>
            ...
            <ul>
                @forelse ($courses as $item)
                    <li
                        class="flex my-2 justify-between bg-slate-100 border border-green-500 rounded-md items-center ">
                        <div class="flex flex-col  p-3">
                            @if ($item->type == 0)
                                ...
                                <div>
                                    ...
                                    <span class="text-gray-500 text-xs">(Total Fees: ₹{{$item->course->getPaidAmount() * $item->course->duration}})/-</span>
                                </div>
                        </div>
                    @else
    ...
    @empty
    <div class="flex">
        <p class="text-6xl font-bold font-sans text-slate-200">Empty Course</p>
    </div>
        @endforelse
...
<div class="bg-white p-3 shadow-sm rounded-sm mt-3 w-1/2">
    ...
    <ul class="list-inside space-y-2">
        ...
        @forelse ($pendingCourses as $item)
            <li class="flex justify-between  bg-slate-100 border border-red-300 rounded-md items-center ">
                    ...
                    <form action="{{ route('paytm.payment') }}" method="post">
                        @csrf
                        <input type="hidden" name="amount" value="@if($item->type==1){{ $item->course->discount_fee }} @else {{$item->course->getPaidAmount()}} @endif">
        ...
        @empty
            <div class="flex">
                <p class="text-6xl font-bold font-sans text-slate-200">Empty Cart</p>
            </div>
        @endforelse
    </ul>
</div>
...
<div class="flex flex-col w-full items-start space-x-2 font-semibold text-gray-900 leading-8 mb-3">
    <span clas="text-green-500">
